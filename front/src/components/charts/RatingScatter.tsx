...
export function RatingScatter({ movies }: Props) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-lg font-semibold text-gray-900 dark:text-white">평점 vs 인기도</h2>
      ...
      <ResponsiveContainer width="100%" height={420}>
        <ScatterChart margin={{ top: 10, right: 30, bottom: 10, left: 0 }}>
          ...
          <XAxis
            ...
            label={{ value: '평점', position: 'insideBottom', offset: -5, fill: '#9ca3af', fontSize: 12 }}
          />
          <YAxis
            dataKey="y"
            type="number"
            name="인기도"
            tick={{ fontSize: 12, fill: '#6b7280' }}
            label={{ value: '인기도', angle: -90, position: 'insideLeft', fill: '#9ca3af', fontSize: 12 }}
          />
          <ZAxis range={[40, 40]} />
