...
export function GenreChart({ movies }: Props) {
  ...
  movies.forEach((m) => {
    m.genres.forEach((g) => {
      ...
      genreMap.set(g, { count: prev.count + 1, ratingSum: prev.ratingSum + m.vote_average })
    })
  })
  ...
  const data = Array.from(genreMap.entries())
    ...
    .sort((a, b) => (metric === 'count' ? b.count - a.count : b.rating - a.rating))

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-gray-900 dark:text-white">장르별 분포</h2>
