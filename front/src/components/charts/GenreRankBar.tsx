...
export function GenreRankBar({ movie, allMovies }: Props) {
  ...
  const data = [
    ...
    ...peers.map((m) => ({ title: m.title, value: m.vote_average, isCurrent: false })),
    ...
    <div>
      ...
      <ResponsiveContainer width="100%" height={Math.max(220, data.length * 36)}>
        ...
        >
          ...
          <YAxis
            ...
            tick={({ x, y, payload }) => (
              <text
                ...
                dy={4}
                textAnchor="end"
                fill={data.find((d) => d.title === payload.value)?.isCurrent ? '#22c55e' : '#9ca3af'}
                fontSize={11}
                fontWeight={data.find((d) => d.title === payload.value)?.isCurrent ? 700 : 400}
              >
          ...
          />
          <ReferenceLine x={movie.vote_average} stroke="#22c55e" strokeDasharray="3 3" />
