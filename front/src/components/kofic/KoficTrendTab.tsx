    ...
    <div className="space-y-14">
      ...
      <div>
        ...
        <ResponsiveContainer width="100%" height={320}>
          <BarChart data={monthlyChart} margin={{ top: 0, right: 30, left: 10, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#374151" opacity={0.3} />
            <XAxis dataKey="name" tick={{ fontSize: 11, fill: '#9ca3af' }} angle={-45} textAnchor="end" height={50} />
            <YAxis tick={{ fontSize: 11, fill: '#6b7280' }} tickFormatter={(v) => `${v}억`} />
            <Tooltip
              contentStyle={tooltipStyle}
              formatter={(v: number, name: string) => {
                if (name === 'sales') return [`${fmtWon(v * 1e8)}원`, '매출액']
                return [v, name]
              }}
      ...
      <div>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={monthlyChart} margin={{ top: 0, right: 30, left: 10, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#374151" opacity={0.3} />
            <XAxis dataKey="name" tick={{ fontSize: 11, fill: '#9ca3af' }} angle={-45} textAnchor="end" height={50} />
            <YAxis tick={{ fontSize: 11, fill: '#6b7280' }} tickFormatter={(v) => `${v}만`} />
            <Tooltip
              contentStyle={tooltipStyle}
              formatter={(v: number) => [`${fmtAudi(v * 1e4)}명`, '관객수']}
            />
            <Line
              type="monotone"
              dataKey="audience"
      ...
      {seasonalChart.length > 0 && (
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mb-6">
            <span className="inline-block w-2.5 h-2.5 rounded-full mr-1" style={{ background: '#4ade80' }} />봄(3-5월)
            <span className="inline-block w-2.5 h-2.5 rounded-full ml-3 mr-1" style={{ background: '#f97316' }} />여름(6-8월)
            <span className="inline-block w-2.5 h-2.5 rounded-full ml-3 mr-1" style={{ background: '#eab308' }} />가을(9-11월)
            <span className="inline-block w-2.5 h-2.5 rounded-full ml-3 mr-1" style={{ background: '#60a5fa' }} />겨울(12-2월)
          </p>
          <ResponsiveContainer width="100%" height={Math.max(280, seasonalChart.length * 48)}>
            <BarChart data={seasonalChart} layout="vertical" margin={{ top: 0, right: 80, left: 10, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#374151" opacity={0.3} horizontal={false} />
              <XAxis type="number" tick={{ fontSize: 11, fill: '#6b7280' }} tickFormatter={(v) => `${v}억`} />
              <YAxis type="category" dataKey="name" width={100} tick={{ fontSize: 11, fill: '#9ca3af' }} />
              <Tooltip
                contentStyle={tooltipStyle}
                formatter={(v: number) => [`${fmtWon(v * 1e8)}원`, '매출액']}
                labelFormatter={(label) => {
      ...
      {seasonalChart.length > 0 && (
        ...
        <div>
          ...
          <ResponsiveContainer width="100%" height={Math.max(280, seasonalChart.length * 48)}>
            <BarChart data={seasonalChart} layout="vertical" margin={{ top: 0, right: 80, left: 10, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#374151" opacity={0.3} horizontal={false} />
              <XAxis type="number" tick={{ fontSize: 11, fill: '#6b7280' }} tickFormatter={(v) => `${v}만`} />
              <YAxis type="category" dataKey="name" width={100} tick={{ fontSize: 11, fill: '#9ca3af' }} />
              <Tooltip
                contentStyle={tooltipStyle}
                formatter={(v: number) => [`${fmtAudi(v * 1e4)}명`, '관객수']}
      ...
      {seasonal.length > 0 && (
        <div>
          <h3 className="text-base font-semibold text-gray-900 dark:text-white mb-6">계절별 요약</h3>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 dark:bg-gray-900 text-gray-500 dark:text-gray-400">
                  ...
                  <th className="px-4 py-3 text-right font-medium">평균 스크린</th>
                  <th className="px-4 py-3 text-left font-medium">1위 영화</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {seasonal.map((s) => (
                  <tr key={s.period} className="hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{fmtWon(s.totalSales)}원</td>
                    <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{fmtAudi(s.totalAudience)}명</td>
                    <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{s.movieCount}편</td>
                    <td className="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{s.avgScreens.toLocaleString()}개</td>
                    <td className="px-4 py-3 text-gray-700 dark:text-gray-300">{s.topMovie}</td>
                  </tr>
                ))}
              </tbody>
            </table>
