    ...
    <div className="space-y-12">
      <div>
        ...
        <ResponsiveContainer width="100%" height={320}>
          <PieChart>
            <Pie data={shareData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={120}
      ...
      <div>
        ...
        <ResponsiveContainer width="100%" height={Math.max(280, entries.length * 28)}>
          <BarChart data={salesAmtData} layout="vertical" margin={{ top: 0, right: 80, left: 10, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#374151" opacity={0.3} horizontal={false} />
            <XAxis type="number" tick={{ fontSize: 11, fill: '#6b7280' }} tickFormatter={(v) => `${(v / 1e4).toFixed(0)}억`} />
            <YAxis type="category" dataKey="name" width={140} tick={{ fontSize: 11, fill: '#9ca3af' }} />
            <Tooltip contentStyle={tooltipStyle} formatter={(v: number) => [`${fmtWon(v * 1e4)}원`, '매출액']} />
            <Bar dataKey="value" radius={[0, 4, 4, 0]}>
              {salesAmtData.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div>
        ...
        <ResponsiveContainer width="100%" height={Math.max(280, entries.length * 28)}>
          <BarChart data={salesAccData} layout="vertical" margin={{ top: 0, right: 80, left: 10, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#374151" opacity={0.3} horizontal={false} />
            <XAxis type="number" tick={{ fontSize: 11, fill: '#6b7280' }} />
            <YAxis type="category" dataKey="name" width={140} tick={{ fontSize: 11, fill: '#9ca3af' }} />
            <Tooltip contentStyle={tooltipStyle} formatter={(v: number) => [`${v}억원`, '누적 매출']} />
