    ...
    <div className="space-y-12">
      ...
      <div>
        ...
        <ResponsiveContainer width="100%" height={Math.max(280, entries.length * 28)}>
          <BarChart data={audiCntData} layout="vertical" margin={{ top: 0, right: 80, left: 10, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#374151" opacity={0.3} horizontal={false} />
            <XAxis type="number" tick={{ fontSize: 11, fill: '#6b7280' }} tickFormatter={(v) => fmtAudi(v)} />
            <YAxis type="category" dataKey="name" width={140} tick={{ fontSize: 11, fill: '#9ca3af' }} />
            <Tooltip contentStyle={tooltipStyle} formatter={(v: number) => [`${v.toLocaleString()}명`, '관객수']} />
      ...
      <div>
        ...
        <ResponsiveContainer width="100%" height={Math.max(280, entries.length * 28)}>
          <BarChart data={audiAccData} layout="vertical" margin={{ top: 0, right: 80, left: 10, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="#374151" opacity={0.3} horizontal={false} />
            <XAxis type="number" tick={{ fontSize: 11, fill: '#6b7280' }} />
            <YAxis type="category" dataKey="name" width={140} tick={{ fontSize: 11, fill: '#9ca3af' }} />
            <Tooltip contentStyle={tooltipStyle} formatter={(v: number) => [`${v}만명`, '누적 관객']} />
      ...
      {trendData.length > 0 && (
        <div>
          ...
          <p className="text-xs text-gray-400 mb-6">신규 진입작 제외 · 양수=증가, 음수=감소</p>
          <ResponsiveContainer width="100%" height={Math.max(200, trendData.length * 36)}>
            <BarChart data={trendData} layout="vertical" margin={{ top: 0, right: 80, left: 10, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#374151" opacity={0.3} horizontal={false} />
              ...
              <Tooltip contentStyle={tooltipStyle} formatter={(v: number) => [`${v.toFixed(1)}%`, '증감률']} />
              <ReferenceLine x={0} stroke="#4b5563" strokeWidth={1.5} />
              <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                {trendData.map((entry, i) => (
                  <Cell key={i} fill={entry.value >= 0 ? '#22c55e' : '#f87171'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
