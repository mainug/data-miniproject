...
export function KoficAllTimeTab() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-48">
      <p className="text-gray-400 text-sm animate-pulse">역대 순위 로딩 중...</p>
    </div>
  )
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex items-center gap-4">
        <h3 className="text-base font-semibold text-gray-900 dark:text-white">역대 흥행 순위</h3>
      ...
      <p className="text-xs text-gray-400 -mt-6">DB에 수집된 기간 내 누적 최대값 기준</p>
      ...
      <ResponsiveContainer width="100%" height={Math.max(400, data.length * 32)}>
        <BarChart data={chartData} layout="vertical" margin={{ top: 0, right: 80, left: 10, bottom: 0 }}>
          ...
          <XAxis
            ...
            tickFormatter={(v) => sortBy === 'audience' ? fmtNum(v) : fmtWon(v)}
          />
          <YAxis type="category" dataKey="name" width={140} tick={{ fontSize: 11, fill: '#9ca3af' }} />
          <Tooltip
            contentStyle={tooltipStyle}
            formatter={(v: number) => [
              sortBy === 'audience' ? `${v.toLocaleString()}명` : fmtWon(v),
              ...
              <tr key={e.rank} className="border-b border-gray-100 dark:border-gray-800">
                <td className="py-3 pr-4">
                  <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold ${
                    e.rank <= 3 ? 'bg-green-500 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400'
                  }`}>
                    {e.rank}
                  </span>
                </td>
                <td className="py-3 pr-4 font-medium text-gray-900 dark:text-white">{e.movieNm}</td>
                <td className="py-3 pr-4 text-right text-gray-500 text-xs">{formatOpenDt(e.openDt)}</td>
                <td className={`py-3 pr-4 text-right font-medium ${sortBy === 'audience' ? 'text-green-600 dark:text-green-400' : 'text-gray-600 dark:text-gray-300'}`}>
                  {fmtNum(e.maxAudiAcc)}명
                </td>
                <td className={`py-3 text-right font-medium ${sortBy === 'sales' ? 'text-green-600 dark:text-green-400' : 'text-gray-600 dark:text-gray-300'}`}>
