...
export function StatBanner({ movies }: Props) {
  ...
  const avgRating = (
    movies.reduce((s, m) => s + m.vote_average, 0) / movies.length
  ).toFixed(1);
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {[
            ...
            >
              ...
              <p
                className={`text-3xl font-bold tracking-tight ${s.accent ? "text-green-500" : "text-gray-900 dark:text-white"}`}
              >
                {s.value}
              </p>
