...
export function BattlePage() {
  ...
  useEffect(() => {
    if (pool.length >= 2) nextRound(pool)
  }, [pool, nextRound])
      ...
      <div className="min-h-screen bg-white dark:bg-[#0a0a0a] text-gray-900 dark:text-white">
        ...
        <div className="flex flex-col items-center justify-center h-[calc(100vh-4rem)] gap-4">
          <p className="text-gray-500 dark:text-gray-400 text-sm">
            {error ?? 'KOFIC 데이터가 부족합니다. 대시보드에서 박스오피스를 먼저 조회해주세요.'}
          </p>
        </div>
      </div>
    )
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="text-sm text-gray-500 dark:text-gray-300">
            최고 연속 정답: <span className="text-green-500 dark:text-green-400 font-bold">{best}</span>
          </div>
          <div className="flex items-center gap-2 text-sm">
            <span className="text-gray-500 dark:text-gray-300">연속 정답</span>
            <motion.span
              key={streak}
              initial={{ scale: 1.6, color: streak === 0 ? '#ef4444' : '#22c55e' }}
        ...
        <div className="grid grid-cols-[1fr_auto_1fr] gap-4 items-center">
          ...
          <div className="flex flex-col items-center gap-2">
            <span className="text-3xl font-black text-gray-300 dark:text-gray-600">VS</span>
          </div>
            ...
            >
              <p
                className={`text-2xl font-extrabold ${
                  resultMsg === '정답!' ? 'text-green-500 dark:text-green-400' : 'text-red-500 dark:text-red-400'
                }`}
              >
                {resultMsg}
              </p>
...
function BattleCard({ movie, side, category, picked, winnerSide, onPick }: CardProps) {
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-gray-500 dark:text-gray-300 mb-5">개봉 {movie.openDt || '미정'}</p>

      {/* 스탯 */}
      <div className="flex flex-col items-center py-4">
        <p className="text-xs text-gray-500 dark:text-gray-300 uppercase tracking-widest mb-2">{category.label}</p>
          ...
          {isRevealed ? (
            <motion.p
              ...
              className={`text-3xl font-extrabold tabular-nums ${
                isWinner ? 'text-green-500 dark:text-green-400' : 'text-gray-400 dark:text-gray-500'
              }`}
            >
