...
import { KoficDateNav } from "../components/kofic/KoficDateNav";
...
import { KoficWeeklyNav } from "../components/kofic/KoficWeeklyNav";
import {
  getYesterday,
  buildShowRange,
  buildWeekendRange,
  getLastMonday,
} from "../components/kofic/koficDate";
...
export function DashboardPage() {
  ...
  // topN 또는 장르/데이터가 바뀌면 yearRange[0]을 확장해서 topN개를 채움
  // (effect 대신 렌더 중 조정 — React 권장 패턴)
  const expandTrigger = `${topN}|${selectedGenres.join(",")}|${movies.length}`;
  const [prevExpandTrigger, setPrevExpandTrigger] = useState("");
  if (movies.length > 0 && expandTrigger !== prevExpandTrigger) {
    ...
    setYearRange((prev) => {
      ...
      return startYear === prev[0] ? prev : [startYear, prev[1]];
    });
  }
  ...
  const { data: derivedData, loading: derivedLoading } =
    useDerivedStats(koficDate);
  ...
  const weeklyShowRange =
    weeklyGb === "0"
      ? buildShowRange(weeklyMonday)
      : buildWeekendRange(weeklyMonday);
  const {
    entries: weeklyEntries,
    loading: weeklyLoading,
    error: weeklyError,
  } = useWeeklyBoxOffice(weeklyShowRange, weeklyGb);
  ...
  const dailyAiPayload = useMemo<AiCommentaryPayload | null>(
    () =>
      entries.length > 0
        ? { type: "daily", date: koficDate, entries: entries.slice(0, 5) }
        : null,
  ...
  const weeklyAiPayload = useMemo<AiCommentaryPayload | null>(
    () =>
      weeklyEntries.length > 0
        ? {
            type: koficPeriod as "weekly" | "weekend",
            showRange: weeklyShowRange,
            entries: weeklyEntries.slice(0, 5),
          }
        : null,
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const trendAiPayload = useMemo<AiCommentaryPayload | null>(
    () =>
      trendData
        ? {
            type: "trend",
            monthly: trendData.monthly as object[],
            seasonal: trendData.seasonal as object[],
          }
        : null,
    [trendData],
  );
  const { commentary: dailyCommentary, loading: dailyAiLoading } =
    useAiCommentary(source === "kofic" && !isWeekly ? dailyAiPayload : null);
  const { commentary: weeklyCommentary, loading: weeklyAiLoading } =
    useAiCommentary(source === "kofic" && isWeekly ? weeklyAiPayload : null);
  const { commentary: trendCommentary, loading: trendAiLoading } =
    useAiCommentary(
      source === "kofic" && koficTab === "trend" && !isWeekly
        ? trendAiPayload
        : null,
    );
                ...
                <div className="max-w-7xl mx-auto px-6 sm:px-10 py-10">
                  <AnimatePresence mode="wait">
                    ...
                    >
                      ...
                      {tmdbTab === "trend" && (
                        <YearTrendChart movies={filtered} />
                      )}
                      {tmdbTab === "analysis" && (
          ...
          >
            ...
            <div className="max-w-7xl mx-auto px-6 sm:px-10 py-10">
              {(isWeekly ? weeklyLoading : koficLoading) ? (
                ...
                <>
                  {!isWeekly &&
                    ["ranking", "sales", "audience"].includes(koficTab) && (
                      <AiCommentaryCard
